...
public class DateUtil<main> {
    ...
    public final static String FORMAT_DATETIME_HH = "yyyy-MM-dd HH:mm";
     ...
     * @param date
     * @return
     * @MethodName: translate2WeekEnumValue
     * @Description: 转换为WeekEnum对应的值
     */
    public static String translate2WeekEnumValue(Date date) {
    ...
    public static int compareStringTime(String DateStr1, String DateStr2) {
        int i = -1000;
        try {
    ...
    public static String getAgeFromBirth(String date) {
        String[] data = date.split("-");
        if (data.length < 3) return "";
        int birth_year = Integer.valueOf(data[0]);
        int birth_month = Integer.valueOf(data[1]);
        int birth_day = Integer.valueOf(data[2]);
        Calendar birthday = new GregorianCalendar(Integer.valueOf(data[0]), Integer.valueOf(data[1]), Integer.valueOf(data[2]));
        Calendar now = Calendar.getInstance();

        int day = now.get(Calendar.DAY_OF_MONTH) - birth_day;
        int month = now.get(Calendar.MONTH) + 1 - birth_month;
        int year = now.get(Calendar.YEAR) - birth_year;
   ...
   /* public static String getAgeToDay(String birthDay) {
     ...
     * 得到二个日期间的间隔分钟数,取绝对值
     */
    public static String getTwoDateHaveMinByAbs(String sj1, String sj2) {
        SimpleDateFormat myFormatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        long day = 0;
        try {
            Date oldDate = myFormatter.parse(sj1);
    ...
    public static long countWeeks(String strDateStar, String strDateEnd) {
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
    ...
    public static void main(String[] args) {
        ...
        String days =  getTwoDay("2019-07-01","2019-08-01");
        System.out.println("ddd==="+days);
        System.out.println("一共有几周："+countWeeks("2019-01-01","2019-08-01"));*/

        String nowHM = getCurrentDateStr(DateUtil.FORMAT_MIN);

        String afterNow = dateToString(addMinutes(new Date(), -50), DateUtil.FORMAT_MIN);
        System.out.println("现在是几点：" + nowHM);
        System.out.println("加减后的时间是几点：" + afterNow);
        String nowTime = DateUtil.getCurrentTime();
        String nowT = "2019-10-16 13:12:00";
        String order = "2019-10-16 11:12:00";
        String min = getTwoDateHaveMin(nowT, order);
        System.out.println("两个时间间隔：" + min);

        String monthAge = getAgeFromBirth("2018-11-09");
        System.out.println("宝宝的月龄：" + monthAge);
        //int i = compareStringTime(nowT,order); // a < b时为-1 ，a>b 时 为1 ，a=b时为0
        //System.out.println("时间对比的结果："+i);
