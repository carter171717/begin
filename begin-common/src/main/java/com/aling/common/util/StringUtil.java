...
public final class StringUtil {
    ...
    public static String cutString(String str, int size) {
        return (isNotEmpty(str) && str.length() > size ? str.substring(0, size) : str);
    }

    /**
     * 卡号显示前8位和后面6位卡号,隐藏掉中间的部分
     *
     * @param cardNo
     * @return
     */
    public static String cardNoReplace(String cardNo) {
        if (isEmpty(cardNo)) {
            return "";
        } else {
            cardNo = cardNo.replaceAll("(\\d{8})\\d+(\\d{6})", "$1***$2");
        }
        return cardNo;
    }
    ...
    //判断日期格式是否不合法 不合法返回true 合法返回FALSE
    public static boolean isNotValidDate(String str, String formatStyle) {
...
// 设置lenient为false. 否则SimpleDateFormat会比较宽松地验证日期，比如2007/02/29会被接受，并转换成2007/03/01
    ...
    public static boolean isStartBigThenEnd(String DATE1, String DATE2) {
        ...
        try {
            ...
            if (dt1.getTime() > dt2.getTime()) {
                System.out.println("起始日期大于结束日期");
                isbig = true;
            } else {
                System.out.println("起始日期小于于结束日期");
            }
    ...
    public static void main(String[] args) {
		/*String dateStr = "2018-04-02";
		String end ="2018-04-03";
		String style = "yyyy-MM-dd";
		boolean isdate =StringUtil.isStartBigThenEnd(dateStr, end);
		System.out.println(AppointStatusEnum.getEnum("2"));*/
